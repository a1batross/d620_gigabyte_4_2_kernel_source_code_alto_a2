...
#define RGK_FLASH_BLINK_SUPPORT
/******************************************************************************/
...
// A:none huangkunming 20140220 @{
#ifdef RGK_FLASH_BLINK_SUPPORT
/* FLASH LED */
char const*const FLASH_LED_FILE
        = "/sys/class/leds/flash/brightness";

char const*const FLASH_TRIGGER_FILE
        = "/sys/class/leds/flash/trigger";

char const*const FLASH_DELAY_ON_FILE
        = "/sys/class/leds/flash/delay_on";

char const*const FLASH_DELAY_OFF_FILE
        = "/sys/class/leds/flash/delay_off";
#endif
// A:}
/* LCD BACKLIGHT */
...
#ifdef RGK_FLASH_BLINK_SUPPORT
static int
blink_flash(int level, int onMS, int offMS)
{
	static int preStatus = 0; // 0: off, 1: blink, 2: no blink
	int nowStatus;
	int i = 0;
...
#ifdef LIGHTS_DBG_ON
	ALOGD("blink_flash, level=%d, onMS=%d, offMS=%d\n", level, onMS, offMS);
#endif
	if (nowStatus == 0) {
       	write_int(FLASH_LED_FILE, 0);
		write_str(FLASH_TRIGGER_FILE, "none");
	}
	else if (nowStatus == 1) {
        //write_int(FLASH_LED_FILE, 255); // default full brightness
		write_str(FLASH_TRIGGER_FILE, "timer");
		while (((access(FLASH_DELAY_OFF_FILE, F_OK) == -1) || (access(FLASH_DELAY_OFF_FILE, R_OK|W_OK) == -1)) && i<10) {
			ALOGD("FLASH_DELAY_OFF_FILE doesn't exist or cannot write!!\n");
			i++;
		}
		write_int(FLASH_DELAY_OFF_FILE, offMS);
		write_int(FLASH_DELAY_ON_FILE, onMS);
	}
	else {
		write_str(FLASH_TRIGGER_FILE, "none");
        write_int(FLASH_LED_FILE, 0); // default full brightness
	}
        ...
        struct light_state_t const* state)
{
	...
	blink_flash(state->color,onMS,offMS);
    return 0;
}
#endif
// A:}
        ...
        struct light_state_t const* state)
{
	...
	// M:none huangkunming 20140220 @{
	#ifdef RGK_FLASH_BLINK_SUPPORT
		set_light_flash(dev, &g_notification);
	#else
    	handle_speaker_battery_locked(dev);
	#endif
	// M:none huangkunming 20140220 @{
