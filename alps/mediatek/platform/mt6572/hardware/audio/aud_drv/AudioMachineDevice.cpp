/************************************************************************
History:
1. chunming.tian@ragentek.com 2013.05.08 BUG_ID:None
   Description: Dual speakers to achieve.
************************************************************************/
...
//add BUG_ID:None tianchunming 20130508(start)
...
//modify BUG_ID:None tianchunming 20130508(start)
//#define SPEAKER_L     1
//#define SPEAKER_LR   2
#define SPEAKER_L     0x0001
#define SPEAKER_LR   0x0011
#define SPEAKER_R     0x0010
//modify BUG_ID:None tianchunming 20130508(end)
namespace android
{
...
{
    ALOGD("AudioMachineDevice AnalogOpen jiabaojun DeviceType = %s", kAudioAnalogDeviceTypeName[DeviceType]);
    //add jiabaojun 20130506 target:control the left channel of speaker when handfree (begin)
   ...
   //add jiabaojun(end)	
    ...
    switch (DeviceType) {
        ...
        case AudioAnalogType::DEVICE_OUT_SPEAKERL:
           ...
           // tell kernel to open device
           //modify BUG_ID:None tianchunming 20130508(start)
           //modify jiabaojun 20130506 target:control the left channel of speaker when handfree (begin)
           ALOGD("[tcm][AudioMachineDevice::AnalogOpen]AudioMTKHardware::is_cit_test==%d", android::AudioMTKHardware::is_cit_test);
           ALOGD("[tcm][AudioMachineDevice::AnalogOpen]AudioMTKHardware::is_right_speaker==%d", android::AudioMTKHardware::is_right_speaker);
           if(true == android::AudioMTKHardware::is_cit_test)
           {
               android::AudioMTKHardware::is_cit_test = false;
               if (false == android::AudioMTKHardware::is_right_speaker) 
	            ioctl(mFd,SET_SPEAKER_ON,SPEAKER_L);
	       else
               {
                    ALOGD("[tcm]AudioMTKHardware::is_right_speaker == true");
	            ioctl(mFd,SET_SPEAKER_ON,SPEAKER_R);
               }
           }
	   else
	   {
	      if (mAudioResourceManager->IsModeIncall() == false) 
	          ioctl(mFd,SET_SPEAKER_ON,SPEAKER_LR);
	      else
	            ioctl(mFd,SET_SPEAKER_ON,SPEAKER_R); //ioctl(mFd,SET_SPEAKER_ON,SPEAKER_R);  	
	   //modify jiabaojun (end)
           }
           //modify BUG_ID:None tianchunming 20130508(end)
            break;
