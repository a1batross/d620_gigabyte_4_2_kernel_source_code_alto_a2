...
static unsigned long next_key_time = 0;
...
*/
...
#ifdef ACCDET_EINT
#else
        msleep(200);   //jiabaojun  reduce the work time of detect accdet
#endif
#endif
        if(PLUG_OUT !=accdet_status)
        {
            ...
            {
                                ACCDET_DEBUG("[Accdet][key] jia PLAY_PAUSE ! \n");
				    //jiabaojun get the time
...
// next, previous, volumeup, volumedown send key in multi_key_detection()
            ...
            {
              //jiabaojun report a key when get two MDKEYs within the time
                                ACCDET_DEBUG("[Accdet][key] jia NEXT ..\n");
	    ...
	    //ALPS00038030:reduce the time of remote button pressed during incoming call
			...
			case SHORT_MD:
				ACCDET_DEBUG("[Accdet] Short press middle (0x%x)\n", multi_key);
                           if(call_status == 0)
				 {
				 //jiabaojun judge the time between two SHORT_MD
                              ACCDET_DEBUG("[Accdet] jia Short press middle  a-b=%d \n", (jiffies - next_key_time));
				    if((jiffies - next_key_time) > 80)
				     {
