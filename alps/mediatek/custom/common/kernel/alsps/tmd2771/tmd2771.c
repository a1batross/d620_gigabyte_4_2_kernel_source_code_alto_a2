...
struct wake_lock tmd2771_ps_lock;
       ...
       unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_THRESHOLD_HIGH:
			    ps_result = atomic_read(&obj->ps_thd_val_high);
			    if(copy_to_user(ptr, &ps_result, sizeof(ps_result)))
				{
					err = -EFAULT;
					goto err_out;
			...
			case ALSPS_GET_PS_THRESHOLD_LOW:
			    ps_result = atomic_read(&obj->ps_thd_val_low);
			    if(copy_to_user(ptr, &ps_result, sizeof(ps_result)))
			    {
					err = -EFAULT;
					goto err_out;
...
/*----------------------------------------------------------------------------*/
static ssize_t tmd2771_show_chipinfo(struct device_driver *ddri, char *buf)
{
	int res;
...
static ssize_t tmd2771_show_als(struct device_driver *ddri, char *buf)
{
	...
	if((res = TMD2771_read_als(TMD2771_obj->client, &TMD2771_obj->als)))
	{
...
static ssize_t tmd2771_show_ps(struct device_driver *ddri, char *buf)
{
	...
	if((res = TMD2771_read_ps(TMD2771_obj->client, &TMD2771_obj->ps)))
	{
		return snprintf(buf, PAGE_SIZE, "ERROR: %d\n", res);
	}
	else
...
static ssize_t tmd2771_show_thdvalue(struct device_driver *ddri, char *buf)
{
	ssize_t res;
	if(!TMD2771_obj)
	{
		...
		return snprintf(buf, PAGE_SIZE, "%d _%d\n", TMD2771_obj->ps_thd_val_high,TMD2771_obj->ps_thd_val_low);     

}


static ssize_t tmd2771_show_0x88(struct device_driver *ddri, char *buf)
{
  u8 databuf[3];
  ...
  ssize_t res;
    ...
    databuf[0]= TMD2771_CMM_INT_LOW_THD_LOW;
     res = TMD2771_i2c_master_operate(TMD2771_obj->client, databuf, 0x201, I2C_FLAG_READ);
	if(res<0)
	{
		return snprintf(buf, PAGE_SIZE, "ERROR: %d\n", res);
	}
	...
	{
        return snprintf(buf, PAGE_SIZE, "databuf[0]=0x%2x_databuf[1]=0x%2x\n", databuf[0],databuf[1]);  
	}
}
static ssize_t tmd2771_show_0x89(struct device_driver *ddri, char *buf)
{
  u8 databuf[3];
...
static ssize_t tmd2771_show_0x8a(struct device_driver *ddri, char *buf)
{
  u8 databuf[3];
...
static ssize_t tmd2771_show_0x8b(struct device_driver *ddri, char *buf)
{
  u8 databuf[3];
...
static DRIVER_ATTR(ps_thd_value,      S_IWUSR | S_IRUGO, tmd2771_show_thdvalue, NULL);
static DRIVER_ATTR(register_0x88,      S_IWUSR | S_IRUGO, tmd2771_show_0x88, NULL);
static DRIVER_ATTR(register_0x89,      S_IWUSR | S_IRUGO, tmd2771_show_0x89, NULL);
static DRIVER_ATTR(register_0x8a,      S_IWUSR | S_IRUGO, tmd2771_show_0x8a, NULL);
static DRIVER_ATTR(register_0x8b,      S_IWUSR | S_IRUGO, tmd2771_show_0x8b, NULL);
static DRIVER_ATTR(chipinfo,      S_IWUSR | S_IRUGO, tmd2771_show_chipinfo, NULL);
...
static struct driver_attribute *tmd2771_attr_list[] = {
    &driver_attr_als,
    &driver_attr_ps, 
    &driver_attr_ps_thd_value,    
    &driver_attr_register_0x88,    
    &driver_attr_register_0x89,    
    &driver_attr_register_0x8a,    
    &driver_attr_register_0x8b,    
...
static int TMD2771_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->als_modulus = (400*100*ZOOM_TIME)/(1*1500);//(1/Gain)*(400/Tine), this value is fix after init ATIME and CONTROL register value
	...
	wake_lock_init(&tmd2771_ps_lock,WAKE_LOCK_SUSPEND,"tmd2771_ps wakelock");//hxh
