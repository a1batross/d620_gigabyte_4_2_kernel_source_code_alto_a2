                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	else  if (tp_ic_major == FN_FW_MAJOR)
	{
		...
		return sprintf(buf, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,TP major :0x%x,TP minor :0x%x,current firmware :0x%x,newest firmware:0x%x,","msg2133A","unkown",i2c_client->addr,tp_ic_major,tp_ic_minor,tp_ic_minor,tp_ic_minor);	
}
