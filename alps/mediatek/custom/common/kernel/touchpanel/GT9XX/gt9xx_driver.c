...
// for TP info
	if (sensor_id_g  == BAOMING_SENSOR_ID)
		...
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","LiHe_new_ITO|JunDa",client->addr);
	else if (sensor_id_g == JUNDA_SENSOR_ID ) 
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","JunDa_ID_CHANGED",client->addr);
	else
