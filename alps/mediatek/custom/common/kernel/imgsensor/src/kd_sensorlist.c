...
#if 1
static int  SUBCAMERA_HW_Reg_Debug( struct file *file, const char *buffer, unsigned long count,
                                                                     void *data)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
...
struct proc_dir_entry *prsubEntry;
    ...
    if (prEntry) {
        ...
        prEntry->write_proc = CAMERA_HW_Reg_Debug;
		    PK_ERR("add /proc/driver/camsensor entry success \n");
    }
    else {
        PK_ERR("add /proc/driver/camsensor entry fail \n");
    }

/*sym add for subsensor debug in 20130704*/
    //Register proc file for sub sensor register debug   
    prsubEntry = create_proc_entry("driver/camsubsensor", 0, NULL);
    if (prsubEntry) {
        prsubEntry->read_proc = CAMERA_HW_DumpReg_To_Proc;
        prsubEntry->write_proc = SUBCAMERA_HW_Reg_Debug;
		    PK_ERR("add /proc/driver/camsubsensor entry success \n");
    }
    else {
